...
jq -e '.offline_required == true and .online_update_default == false and .variant_count == 4 and (.media_profiles | length == 5)' \
  "${repo_root}/release/generated/installer-profile.json" >/dev/null
jq -e '.installer.media_profiles == ["online", "kde", "gnome", "niri-dms", "niri-noctalia"]' \
  "${repo_root}/release/generated/release-metadata.json" >/dev/null
